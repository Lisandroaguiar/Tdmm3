...
Amplitude volumenBase;
float moverCuadrados=0;
int numDeImagen;
PImage muerte[];
String val;
...
int tiempoDelay;
int marcaDeTiempoTutorial;
int tiempoFinalTutorial=10000; //TIEMPO QUE TARDA EN CERRAR DESPUES DE PERDER  1 segundo=1000
int tiempoTutorial;
...
void setup()
{
  ...
  volumenBase= new Amplitude (this);
...
void draw() {
  ...
  if (estado=="tutorial") {
    ...
    yaEligio=false;
    push();
    moverCuadrados= volumenBase.analyze()*2000;
    tiempo=(11000-tiempoTutorial)/1000;
    println(moverCuadrados, volumenBase.analyze());
    fill(0, 255, 40);
    rect(width/2-400, height/2+moverCuadrados, 200, 200);
    rect(width/2+200, height/2+moverCuadrados, 200, 200);
    fill(255);
    rect(width/2-400, height/2+300, 800, 20);

    pop();
    if (moverCuadrados<0.02) {
      tiempoTutorial=millis()-marcaDeTiempoTutorial;
      if (tiempoTutorial>tiempoFinalTutorial) {
        marcaDeTiempoTutorial=millis();
      }
      push();
      fill(255, 0, 0);
      textSize(256);
      text(tiempo, width/2-100, 400);
      println(tiempo);
      pop();
    }
    if (tiempo<1 && moverCuadrados<0.002) {
      estado="jugador";
  ...
  if (estado=="perdiste") {
    ...
    rect(width/2-700, height/4-100+moverPerdiste*50, 200, 300-moverPerdiste*300);
    rect(width/2+500, height/4-100+moverPerdiste*50, 200, 300-moverPerdiste*300);
    noFill();
    strokeWeight(10);
    stroke(255, 0, 0);
    line(18, 28+moverPerdiste*50, 218, 99+moverPerdiste*50);
    line(1219, 99+moverPerdiste*50, 1419, 28+moverPerdiste*50);
